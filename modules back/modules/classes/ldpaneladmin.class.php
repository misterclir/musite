		...
		private function optionLoadSincronize($type)
		{
			...
			switch($type)
			{
				case 1:
					...
					{
      		if(time() > strtotime($findVip->dateend))
						{
		...
		private function optionLoadManageVip($type)
		{
			...
			switch($type)
			{
				case 1:
					if($_GET['Write'] == true)
					{
						if(empty($_POST['account']) == true)
							return $ldTpl->set("RESULTTPL", "<div class=\"qdestaques\">".LDPA_FILL_LOGIN."</div>");
						if(is_numeric($_POST['days']) == false)
							return $ldTpl->set("RESULTTPL", "<div class=\"qdestaques\">".LDPA_MVIPS_TEXT_INVALID_DAYS."</div>");
                        ...
                        if(mssql_num_rows($findAccountQ) == 0)
                        	return $ldTpl->set("RESULTTPL", "<div class=\"qdestaques\">".LDPA_INVALID_LOGIN."</div>");
						elseif((int)$findAccountVip->type > 0)
							return $ldTpl->set("RESULTTPL", "<div class=\"qdestaques\">".LDPA_MVIPS_TEXT_ALREADY_HAVE_VIP."</div>");
						else
						{
							if(MUSERVER_TEAM == 1 && VESION_MUSERVER >= 6)
							{
								$expireDate = "DATEADD(day, ".$_POST['days'].", getdate())";
								$this->query("UPDATE ".$TABLES_CONFIGS['WEBVIPS']['database'].".dbo.".$TABLES_CONFIGS['WEBVIPS']['table']." SET ".$TABLES_CONFIGS['WEBVIPS']['columnType']." = ".$_POST['flat'].", ".$TABLES_CONFIGS['WEBVIPS']['columnDateEnd']." = ".$expireDate." WHERE ".$TABLES_CONFIGS['WEBVIPS']['columnUsername']." = '".$_POST['account']."'");
                            }
                            else
                            {
				...
				case 3:
					if($_GET['Write'] == true)
					{
						...
						if(MUSERVER_TEAM == 1 && VESION_MUSERVER >= 6)
                        {
                        	$findAccountVipQ = $this->query("SELECT ".$TABLES_CONFIGS['WEBVIPS']['columnType']." as type, ".$TABLES_CONFIGS['WEBVIPS']['columnDateEnd']." as dateExpire FROM ".$TABLES_CONFIGS['WEBVIPS']['database'].".dbo.".$TABLES_CONFIGS['WEBVIPS']['table']." WHERE ".$TABLES_CONFIGS['WEBVIPS']['columnUsername']."='". $_POST['account'] ."'");
                        }
                        else
                        {
						...
						else
						{
                            if($_GET['subWrite'] == false)
                            {
                                ...
                                {
	                                $dateEndArgs   = array(
	                                                    "day" => date_format($findAccountVip->dateExpire, 'j'),
	                                                    "month" => date_format($findAccountVip->dateExpire, 'n'),
	                                                    "year" => date_format($findAccountVip->dateExpire, 'Y'),
	                                                    "hour" => date_format($findAccountVip->dateExpire, 'h'),
	                                                    "minutes" => date_format($findAccountVip->dateExpire, 'i'),
	                                                    "seconds" => date_format($findAccountVip->dateExpire, 's'),
	                                                    );  
	                            }
	                            else
	                            {
                            ...
                            else
                            {
                                ...
                                $tmpExpireDate = date('m/d/Y H:i:s', $tmpTimestampEnd);
                                
                                if(MUSERVER_TEAM == 1 && VESION_MUSERVER >= 6)
                                {
                                	$this->query("UPDATE ".$TABLES_CONFIGS['WEBVIPS']['database'].".dbo.".$TABLES_CONFIGS['WEBVIPS']['table']." SET ".$TABLES_CONFIGS['WEBVIPS']['columnType']." = ".$_POST['flat'].", ".$TABLES_CONFIGS['WEBVIPS']['columnDateEnd']." = '{$tmpExpireDate}'  WHERE ".$TABLES_CONFIGS['WEBVIPS']['columnUsername']." = '".$_POST['account']."'");
                                }
                                else
                                {
