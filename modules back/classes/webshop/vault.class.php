...
if ( class_exists( "Vault" ) == false ) {
	class Vault extends ldMssql{
		...
		{ 
            $getLenghts = $this->query("USE ".DATABASE_ACCOUNTS." SELECT [length] FROM [syscolumns] WHERE OBJECT_NAME([id]) = 'warehouse' AND [name] = 'Items';");
			...
			$SQL_Q = $this->query("SELECT 1 FROM ".DATABASE_ACCOUNTS.".dbo.warehouse WHERE Accountid='". $_SESSION['LOGIN'] ."'");
			if(mssql_num_rows($SQL_Q) == 0) $this->query("INSERT INTO ".DATABASE_ACCOUNTS.".dbo.warehouse (AccountID, Items, Money, EndUseDate, DbVersion, pw) VALUES ('". $_SESSION['LOGIN'] ."', 0x".str_pad( "", $this->Varbinary*2, "F" ).", 0, GetDate(), ".constant("SYSTEM_DBVERSION").", 0);");
			$SQL_Q = $this->query("DECLARE @vault varbinary(". $this->Varbinary ."); SELECT @vault = items FROM ".DATABASE_ACCOUNTS.".dbo.warehouse WHERE AccountID='".$_SESSION['LOGIN']."' " . (constant("ENCGAMES_S6") === true ? " AND VaultID = 1" : NULL) . "; PRINT @vault;");
		...
		public function WriteVault() 
		{
			...
			$SQL_Q = $this->query("UPDATE ".DATABASE_ACCOUNTS.".dbo.warehouse SET items = 0x".$this->Vault_End_Process." WHERE AccountID='".$_SESSION['LOGIN']."' " . (constant("ENCGAMES_S6") === true ? " AND VaultID = 1" : NULL) . ";");
