...
if ( class_exists( "FinishBuy" ) == false ) {
    class FinishBuy extends ldMssql {
     // public $End_Price, $Price, $Price_Level, $Price_Adicional, $Price_Skill, $Price_Luck, $Price_Ancient, $Price_OpExc, $ProductID, $Item_Level, $Item_Option, $Item_Ancient, $Item_Skill, $Item_Luck, $Item_OpExc_1, $Item_OpExc_2, $Item_OpExc_3, $Item_OpExc_4, $Item_OpExc_5, $Item_OpExc_6;
        ...
        { 
            ...
            $SQL_Q = $this->query("SELECT ConnectStat FROM ".DATABASE_ACCOUNTS.".dbo.MEMB_STAT WHERE memb___id='". $_SESSION['LOGIN'] ."'");
            ...
            if($this->buyKit == false)
            {
                ...
                if($tempOptExcCheck > MAX_OPTIONS_WITH_GOLD) exit(Print_error("<ul><li>Por favor, Selecione apenas ".MAX_OPTIONS_WITH_GOLD." opções excelentes ao comprar com ".GOLDNAME.".</li></ul>")); 
            
                if (isset($_GET['PAYWITHCASH'])) 
                {
                    $this->Calc_Price2();
                    $this->Check_Cashs_Amount();
                }
                else
                {
                    $this->Calc_Price();
            ...
            if($this->buyKit == false)
			{
				    if (isset($_GET['PAYWITHCASH']) == TRUE) {
                        $this->ChargeCash();
                    }
                    else
                    {
        ...
        {
            global $ODBC;
            $ODBC_Q = $ODBC->query("SELECT * FROM [". DB_SHOP_SQL ."].[dbo].[Items] WHERE Number='".$this->ProductID."'");
            ...
            */
			...
			elseif(isset($_GET['PAYWITHCASH']) == TRUE)
            {
                $tempOptExcCheck = 0;
                if($this->Item_OpExc_1 == 'true') $tempOptExcCheck++;
                if($this->Item_OpExc_2 == 'true') $tempOptExcCheck++;
                ...
                if($this->Item_OpExc_5 == 'true') $tempOptExcCheck++;
                if($this->Item_OpExc_6 == 'true') $tempOptExcCheck++;  
                if($tempOptExcCheck > MAX_OPTIONS_WITH_CASH) exit(Print_error("<ul class=\"listNone\"><li>Desculpe, voc&ecirc; pode selecionar apenas ". MAX_OPTIONS_WITH_CASH ." op&ccedil;&otilde;es excelentes pagando com ". CASHNAME .".</li></ul>")); 
            }
            ...
            $this->CashPrice = $ODBC_R->CashPrice;
            $this->IsGoldItem = $ODBC_R->GoldItem;
            $this->IsCashItem = $ODBC_R->CashItem;
            ...
            $this->maxOptExcSel = MAX_OPTIONS_WITH_GOLD;            
        }
		...
		private function Calc_Price2()
        {
            $this->End_Price2 = 0;
            $this->End_Price2 += $this->CashPrice;
            $this->End_Price2 += ($this->Item_Level * $this->Price_Level);
            $this->End_Price2 += ($this->Item_Option * $this->Price_Adicional);
            $this->End_Price2 += ($this->Item_Ancient * $this->Price_Ancient);
            $this->End_Price2 += ($this->Item_Skill == "true" ? $this->Price_Skill : 0);
            $this->End_Price2 += ($this->Item_Luck == "true" ? $this->Price_Luck : 0);
            $this->End_Price2 += ($this->Item_OpExc_1 == "true" ? $this->Price_OpExc : 0);
            $this->End_Price2 += ($this->Item_OpExc_2 == "true" ? $this->Price_OpExc : 0);
            $this->End_Price2 += ($this->Item_OpExc_3 == "true" ? $this->Price_OpExc : 0);
            $this->End_Price2 += ($this->Item_OpExc_4 == "true" ? $this->Price_OpExc : 0);
            $this->End_Price2 += ($this->Item_OpExc_5 == "true" ? $this->Price_OpExc : 0);     
            $this->End_Price2 += ($this->Item_OpExc_6 == "true" ? $this->Price_OpExc : 0);   
            $this->End_Price2 += ($this->Item_JH != "00" ? $this->Price_JH : 0);                    
            $this->End_Price2 += ($this->Item_Refine == "true" ? $this->Price_Refine : 0);        
            $this->End_Price2 += ($this->Item_Socket_Slot_1 == "true" ? $this->Price_Socket : 0); 
            $this->End_Price2 += ($this->Item_Socket_Slot_2 == "true" ? $this->Price_Socket : 0); 
            $this->End_Price2 += ($this->Item_Socket_Slot_3 == "true" ? $this->Price_Socket : 0); 
            $this->End_Price2 += ($this->Item_Socket_Slot_4 == "true" ? $this->Price_Socket : 0); 
            $this->End_Price2 += ($this->Item_Socket_Slot_5 == "true" ? $this->Price_Socket : 0);  
            if($_SESSION['COUPON_ACTIVE'] == true) $this->End_Price2 = ceil((( $this->End_Price2 / 100 ) * (100 - $_SESSION['COUPON_PERCENT'])));
        }
        private function Check_Golds_Amount()
        {
            $SQL_Q = $this->query("SELECT ".GOLDCOLUMN." FROM ".DATABASE_ACCOUNTS.".dbo.".GOLDTABLE." WHERE ".GOLDMEMBIDENT." = '". $_SESSION['LOGIN'] ."'");
            $SQL = mssql_fetch_row($SQL_Q);
			if($this->IsGoldItem == False) exit(Print_error("<ul><li>Desculpe, Item indisponivel para compra com ".GOLDNAME.".</li></ul>"));
            elseif($SQL[0] < $this->End_Price) exit(Print_error("<ul><li>Desculpe, essa compra n&atilde;o pode ser realizada, pois seu saldo de ".GOLDNAME." &eacute; insuficiente.</li></ul>"));
        }
        private function Check_Cashs_Amount()
        {
            $SQL_Q = $this->query("SELECT ".CASHCOLUMN." FROM ".DATABASE_ACCOUNTS.".dbo.".GOLDTABLE." WHERE ".GOLDMEMBIDENT." = '". $_SESSION['LOGIN'] ."'");
            $SQL = mssql_fetch_row($SQL_Q);
			if($this->IsCashItem == False) exit(Print_error("<ul><li>Desculpe, Item indisponivel para compra com ".CASHNAME.".</li></ul>"));
            if($SQL[0] < $this->End_Price2) exit(Print_error("<ul><li>Desculpe, essa compra n&atilde;o pode ser realizada, pois seu saldo de ".CASHNAME." &eacute; insuficiente.</li></ul>"));
        }
        private function ChargeGold()
        {
            $SQL_Q = $this->query("UPDATE ".DATABASE_ACCOUNTS.".dbo.".GOLDTABLE." SET ".GOLDCOLUMN." = ".GOLDCOLUMN."-".$this->End_Price." WHERE ".GOLDMEMBIDENT." = '". $_SESSION['LOGIN'] ."' AND ".GOLDCOLUMN." >= ".$this->End_Price."; select @@rowcount as rows;");
            $SQL_R = mssql_fetch_object($SQL_Q);
            if((int)$SQL_R->rows == 0) exit(Print_error("<ul><li>Erro ao cobrar pelo item.</li></ul>"));
        }
        private function ChargeCash()
        {
            $SQL_Q = $this->query("UPDATE ".DATABASE_ACCOUNTS.".dbo.".GOLDTABLE." SET ".CASHCOLUMN." = ".CASHCOLUMN."-".$this->End_Price2." WHERE ".GOLDMEMBIDENT." = '". $_SESSION['LOGIN'] ."' AND ".CASHCOLUMN." >= ".$this->End_Price2."; select @@rowcount as rows;");
            $SQL_R = mssql_fetch_object($SQL_Q);
            if((int)$SQL_R->rows == 0) exit(Print_error("<ul><li>Erro ao cobrar pelo item.</li></ul>"));
        }
